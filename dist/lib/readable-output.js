...
function normalizeForDedup(kind, body) {
...
function isBoxDrawingLine(text) {
    if (/[\u2500-\u257F\u2580-\u259F]/u.test(text)) {
        return true;
    }
...
function isTerminalChromeLine(text) {
    const lower = text.toLowerCase();
    return (lower.includes("@filename") ||
...
function isBrokenResidueLine(text) {
...
function isErrorLine(text) {
    return /\b(error|erreur|failed|failure|traceback|enoent|eacces|exception|incorrect function|os error)\b/i.test(text);
}
...
function isHumanReadableLine(text) {
    const normalized = stripBulletPrefix(text);
...
function isPathLine(text) {
    ...
    if (/^[A-Za-z]:\\[\w\\.\-()[\] ]+$/.test(trimmed))
        return true;
    if (/^\.{0,2}[\\/][\w/\\.\-()[\]@ ]+$/.test(trimmed))
        return true;
    if (/^[\w.\-/\\()[\]@]+\.\w{1,5}$/.test(trimmed) && /[/\\]/.test(trimmed))
        return true;
    if (/^[\w.\-/\\()[\]@]+[\\/]$/.test(trimmed))
        return true;
    return false;
...
function isCodeBlockLine(line) {
    ...
    if (/;\s*$/.test(trimmed))
        signals += 1;
    if (/^\s*(import|export)\s+.+\s+from\s+['"]/.test(trimmed))
        signals += 2;
    if (/^\s*(const|let|var)\s+\w+\s*[:=]/.test(trimmed))
        signals += 2;
    if (/^\s*function\s+\w+\s*\(/.test(trimmed))
        signals += 2;
    if (/^\s*(class|interface)\s+\w+/.test(trimmed) && /[{]/.test(trimmed))
        signals += 2;
    if (/^\s*return\s+[^;]+;?\s*$/.test(trimmed))
        signals += 1;
    if (/=>\s*[{(]/.test(trimmed))
        signals += 1;
    if (/^\s*(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER)\s+/i.test(trimmed) &&
        /\b(FROM|INTO|SET|TABLE|VALUES|WHERE)\b/i.test(trimmed)) {
        signals += 2;
    }
    return signals >= 2;
}
function splitAroundInlineCommand(text) {
    const trimmed = text.trim();
    if (!trimmed) {
        return [];
    }
...
function splitAroundInlinePaths(text) {
    const trimmed = text.trim();
    if (!trimmed ||
...
function appendGroupedBlock(grouped, candidate) {
...
//# sourceMappingURL=readable-output.js.map