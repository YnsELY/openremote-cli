...
export async function setupCommand() {
    ...
    const missingCliTools = [];
    if (!codexCheck.ok)
        missingCliTools.push(codexCheck.detail);
    if (!claudeCodeCheck.ok)
        missingCliTools.push(claudeCodeCheck.detail);
    if (!qwenCheck.ok)
        missingCliTools.push(qwenCheck.detail);
    if (missingCliTools.length > 0) {
        log.card("Some CLI tools are missing", ["Install the following tools:", "", ...missingCliTools, "", "Then run openremote setup again."], "warning");
    }
    log.card("Security notice", [
